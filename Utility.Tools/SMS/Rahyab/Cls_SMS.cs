...
public class Cls_SMS
{
    ...
    public class ClsStatus
    {
        ...
        public async Task<ArrayList> GetSMSStatusAsync(string followongCode)
        {
            try
            {
                string[] S = new string[1];
                ...
                objStringBuilder.Append("<!DOCTYPE smsStatusPoll PUBLIC \"-//PERVASIVE//DTD CPAS 1.0//EN\" \"http://www.pervasive.ir/dtd/Cpas.dtd\" []>");
                objStringBuilder.Append(Environment.NewLine);
                objStringBuilder.Append("<smsStatusPoll company=\"" + RahyabParameters.Shenase + "\">");
                objStringBuilder.Append(Environment.NewLine);
                objStringBuilder.Append("<batch batchID=\"" + $"{RahyabParameters.Shenase }+{followongCode}" + "\" />");
                objStringBuilder.Append(Environment.NewLine);
                objStringBuilder.Append("</smsStatusPoll>");
                ...
                byte[] byt = System.Text.Encoding.UTF8.GetBytes(RahyabParameters.Username + ":" + RahyabParameters.Password);
                objWebRequest.Headers.Add("authorization", "Basic " + Convert.ToBase64String(byt));
                Stream objStream = await objWebRequest.GetRequestStreamAsync();
                StreamWriter objStreamWriter = new StreamWriter(objStream);
                objStreamWriter.Write(dataToPost);
                objStreamWriter.Flush();
                objStreamWriter.Close();
                ...
                WebResponse objWebResponse = await objWebRequest.GetResponseAsync();
                Stream objResponseStream = objWebResponse.GetResponseStream();
                StreamReader objStreamReader = new StreamReader(objResponseStream);
                StrXML = await objStreamReader.ReadToEndAsync();
                objStreamReader.Close();
                objResponseStream.Close();
                objWebResponse.Close();
        ...
        public  ArrayList GetSMSStatus(string followongCode)
        {
            try
            {
                string[] S = new string[1];
    ...
    public class ClsGetRemain
    {
        public long GetRemainCredit()
        {
            try
            {
                ...
                objStringBuilder.Append("<?xml version=\"1.0\"?>");
                objStringBuilder.Append(Environment.NewLine);
                objStringBuilder.Append("<getUserBalance company=\"" + RahyabParameters.Shenase + "\" />");
                string dataToPost = objStringBuilder.ToString();
